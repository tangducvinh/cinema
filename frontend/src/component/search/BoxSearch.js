...
const BoxSearch = () => {
    ...
    const fecthDataMovieShow = async() => {
        ...
        if (response?.success) {
            setDataMovieShow(response.data.map(item => ({id: item._id, name: item.original_title})))
        }
    }
    ...
    const fecthListShow = async(data) => {
        ...
        if (response.success) {
            setListshow(response.data.map(item => ({id: item._id, time: moment(item.begin_time).format('HH:mm')})))
        }
    }
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            listDay.push({value: `${currentDay.getFullYear()}-${currentDay.getMonth() + 1}-${currentDay.getDate() + i}`, text: `${tuesday}, ${currentDay.getDate() + i}-${currentDay.getMonth() + 1}-${currentDay.getFullYear()}`})
        }
    ...
    return (
        <div className="flex items-center shadow-lg">
            <div className="flex items-center pl-3 pr-1 py-4 rounded-l-sm">
                ...
                <div className="mr-2 flex items-center text-sm">
                    <label className="bg-main text-white px-[6px] text-[12px] rounded-full mr-1">2</label>
                    <select className="w-[250px] outline-none cursor-pointer" value={valueDay} onChange={(e) => setValueDay(e.target.value)}>
