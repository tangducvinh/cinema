...
const ItemUserInfor = ({ name, email, phone, role, image, _id }) => {
  ...
  return (
    <ul
      className={clsx(
        "flex items-center text-[15px] px-4 w-full mt-3 rounded-md border-b-2 border-l-4  py-1 shadow-md",
        { "border-l-main": +role === 9 },
        { "border-l-green-500": +role === 3 },
        { "border-l-blue-500": +role === 7 }
      )}
    >
      ...
      <li className="flex-1 font-medium line-clamp-1 pr-2">
        {email.length < 28 ? email : `${email.slice(0, 26)}...`}
      </li>
      <li className="flex-1 font-medium">{phone}</li>
      <li className="flex-1 font-medium">
        {listRole.find((item) => item.value === +role).name}
      </li>
      <li className="cursor-pointer" onClick={handleEditUser}>
        <MdOutlineRestorePage size="22px" />
      </li>
      <li className="ml-2 cursor-pointer" onClick={handleDeleteUser}>
