...
const getAllShow = async(req, res) => {
    try {
        ...
        if (title) {
            console.log('hello 2')
            query = {...query, 'movieId.$original_title': {$regex: title, $options: 'i'}}
        }
        ...
        const response = await Show.find({...query, begin_time: {'$gte': minTime, '$lt': maxTime}}).sort({'begin_time': 1}).populate(
            [
                {
                    path: 'movieId',
